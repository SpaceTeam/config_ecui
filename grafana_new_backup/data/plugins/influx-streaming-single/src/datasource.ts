        ...
        const IntervalId = setInterval(() => {
          if (Date.now() - lastQuery > query.interval) {
            ...
            const queryText = getTemplateSrv().replace(
              'select mode("value") as "value" from "states" where time>now()-100ms and ("key"=\'engine_thrust_x\' or "key"=\'engine_thrust_y\' or "key"=\'engine_thrust_z\') group by time($__interval), "key" fill(none) order by time desc limit 3',
              options.scopedVars
            );
